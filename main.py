...
fromFile = True
fileName = os.path.dirname(os.path.abspath(__file__)) + "/problems/berlin52.tsp"
...
if fromFile:
    ...
    for i in range(len(cityList)):
        ...
        for j in range(len(cityList)):
            distanceToI.append(
                math.sqrt((abs(cityList[i][0] - cityList[j][0])) ** 2 + abs(cityList[i][1] - cityList[j][1]) ** 2))
...
def generation(population):
    ...
    for i in range(nr_of_children - nr_of_cloned_children):
        population[survivors + i + nr_of_cloned_children] = population[random.choice(range(survivors))].reproduce(
...
def run():
    ...
    for i in range(nrOfGenerations - 1):
        ...
        if printValues is True:
            best = findbest(population)
            print(
                "Best solution in population " + str(i + 2) + ": length: " + str(best.cost) + ", solution: " + str(
                    best.gene))
